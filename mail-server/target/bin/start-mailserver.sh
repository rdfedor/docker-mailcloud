...
DEFAULT_VARS["ENABLE_SQL"]="${ENABLE_SQL:="0"}"
DEFAULT_VARS["SQL_DRIVER"]="${SQL_DRIVER:="sqlite"}"
DEFAULT_VARS["SQL_CONNECT"]="${SQL_CONNECT:="/var/mail/database.sqlite"}"
##########################################################################
# << DEFAULT VARS
##########################################################################
...
function register_functions() {
	...
	if [ "$ENABLE_SQL" = 1 ];then
		_register_setup_function "_setup_dovecot_sql"
	fi
...
function _setup_dovecot_local_user() {
	...
	if [ -f /tmp/docker-mailserver/postfix-accounts.cf -a "$ENABLE_LDAP" != 1 -a "$ENABLE_SQL" != 1 ]; then
...
function _setup_dovecot_sql() {
	notify 'task' 'Setting up Dovecot SQL'

	notify 'inf' "Applying updates to /etc/dovecot/dovecot-sql.conf.ext"
...
EOF
	...
	[ -f /etc/postfix/virtual_mailbox_maps.cf ] && \
		postconf -e "virtual_mailbox_maps = ${SQL_DRIVER}:/etc/postfix/virtual_mailbox_maps.cf" || \
		notify 'inf' "==> Warning: /etc/postfix/virtual_mailbox_maps.cf not found"

	[ -f /etc/postfix/virtual-mailbox-domains.cf ] && \
		postconf -e "virtual_mailbox_domains = ${SQL_DRIVER}:/etc/postfix/virtual-mailbox-domains.cf" || \
		notify 'inf' "==> Warning: /etc/postfix/virtual-mailbox-domains.cf not found"

	[ -f /etc/postfix/virtual-alias-maps.cf ] && \
		postconf -e "virtual_alias_maps = ${SQL_DRIVER}:/etc/postfix/virtual-alias-maps.cf" || \
		notify 'inf' "==> Warning: /etc/postfix/virtual-alias-maps not found"

	# Have Dovecot query our database, and not system users, for authentication.
	sed -i "s/\!include auth-passwdfile.inc/#\!include auth-passwdfile.inc/g"  /etc/dovecot/conf.d/10-auth.conf
	sed -i "s/#\!include auth-sql.conf.ext/\!include auth-sql.conf.ext/g"  /etc/dovecot/conf.d/10-auth.conf

	/etc/cron.daily/update-postgrey-whitelist.sh
}
...
function _setup_spoof_protection () {
	...
	[ "$ENABLE_LDAP" = 1 ] \
		...
		|| [ "$ENABLE_SQL" = 1 ] \
		&& postconf -e "smtpd_sender_login_maps=${SQL_DRIVER}:/etc/postfix/sender-login-maps.cf" \
